...
append_copula <- function(v, ucond, tau, cop, QY = identity,
    ...
    initfit <- fitbicop_lh(ucond[, chainlen], vcond, families=cop)
    ...
    if (refit_all) {
        ...
        ## Parameter space
        paramspace <- cparspace(c(prev_cops, cop))
        bnds <- cparspace(c(prev_cops, cop), fn = FALSE)
    } else {
        cparstart <- initfit$cpar
        paramspace <- cparspace(cop)
    ...
    return(list(cops = copchain,
                cpars = cparvec2cpar(thetahat, cparlen),
                score = res$minimum))
}
